...
public class MeshTrail : MonoBehaviour
{
    ...
    [Header("Shader Reketed")]                           //셰이더 관련 설정
    ...
    public float shaderVarRefreshRate = 0.5f;            //셰이더 효과가 업데이트 되는 시간 간격

    private SkinnedMeshRenderer[] skinnedMeshRenderers;        //캐릭터의 3D 모델을 랜더링 하는 컴포넌트들
    ...
    IEnumerator ActivateTrail(float timeActivated)
    {
        ...
        while(timeActivated > 0)
        {
            ...
            if(skinnedMeshRenderers == null) //스킨드 메시 렌더러 컴포넌트들을 가져옴
                skinnedMeshRenderers = positionToSpawn.GetComponentsInChildren<SkinnedMeshRenderer>();

            for(int i = 0; i < skinnedMeshRenderers.Length; i++)     //각 메시 렌더러에 대한 잔상 생성
            {
